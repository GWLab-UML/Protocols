...
2. Open terminal and install jupyter notebooks: `pip3 install jupyter notebook jupyter-client`
3. Then, type `R` to code in R
4. Install the IRkernel: `install.packages("IRkernel")` and enter `1` to select the 0-Cloud CRAN mirror
5. Register the kernel: `IRkernel::installspec()`
...
### On Unity:
(with Anaconda)
1. Run `module load conda/latest` in terminal
...
4. but if the kernel doesn't show up in the jupyter notebook, just run `install.packages("IRkernel")` in R
