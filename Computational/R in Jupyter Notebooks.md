...
2. Open terminal and install jupyter notebooks: 
```pip3 install jupyter notebook jupyter-client```
1. Then, type `R` to code in R
2. Install the IRkernel: ```install.packages("IRkernel")``` and enter `1` to select the 0-Cloud CRAN mirror
3. Register the kernel: ```IRkernel::installspec()```
