...
#@ String (label = "Image type ", style = "list", choices = {"jpg", "JPG","jpeg", "JPEG", "tif"}) ImageFormat
...
//Initialize and open images as a stack.
	...
	File.openSequence(ImageDirectory, "filter="+ImageFormat);
	run("Set Scale...", "distance=0 known=0 unit=pixel");
	rename("temp");
	waitForUser("You may crop or otherwise adjust the image stack before proceeding. \n For example, crop to isolate shell or calibration regions of the images. \n When finished or if no adjustment is desired press OK to continue.");
//

// The specified SIOX segmentor file created by using the 
// ShellAreaslTrainSIOXBatch macro is used to create a stack of the masks 
// of the original images.  Each mask is then analyzed individually with the 
// Analyze Particles ImajeJ routine.
	...
	for (i=1; i<NumberOpenImages; i++) {
	    ...
	    // Feret maximum diameter lines are drawn.
...
//Create stack of original and analyzed images, 
//and delete original and mask stacks.
...
// Configure the Results table.  The table contains two columns related to shell 
// shape: Min/Max is the ratio of the minimum Feret diameter to the maximum 
// Feret diameter; Circularity is an Imagej-defined variable that relates the 
// object area to its circumference = 4pi(area/perimeter^2), ranging from 
// 1 to zero between a perfect circle and a straight line.
